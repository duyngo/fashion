...
class DUP_PRO_Build_Progress
{
    ...
    public function set_build_mode($set_for_schedule = false)
    {
        ...
        if ($this->current_build_mode == -1) {
			...
			if($set_for_schedule) {
				$build_mode = $global->archive_build_mode_schedule;
				$build_compression = $global->archive_compression_schedule;
			 } else {
...
class DUP_PRO_Package
{
    ...
    public $ExeSize;
    public $ZipSize = 0;
    //public $ZipMode; -- this was redundant. Information stored in $this->build_progress->current_build_mode
    ...
    public function create_scan_report()
    {
        ...
        DUP_PRO_Log::profile('BUILD SCAN REPORT', true);
        ...
        try {
            ...
            DUP_PRO_Log::profile('BUILD SCAN REPORT', false);
        } catch (Exception $ex) {
    ...
    public function save()
    {
        ...
        if ($this->ID == -1 || empty($this->ID)) {
            DUP_PRO_LOG::profile('Package core save', true);
            /* @var $global DUP_PRO_Global_Entity */
            $global = DUP_PRO_Global_Entity::get_instance();
            ...
            DUP_PRO_LOG::profile('Package core save', false);
...
//					$schedule->last_run_status	 = DUP_PRO_Package_Build_Outcome::SUCCESS;
    ...
    public static function set_manual_template_from_post($post = null)
    {
        if (isset($post)) {
            ...
            $filter_dirs = isset($post['filter-dirs']) ? DUP_PRO_Archive::parseDirectoryFilter($post['filter-dirs']) : '';
            $filter_sites = !empty($post['mu-exclude']) ? $post['mu-exclude'] : '';
            $filter_exts = isset($post['filter-exts']) ? DUP_PRO_Archive::parseExtensionFilter($post['filter-exts']) : '';
            $filter_files = isset($post['filter-files']) ? DUP_PRO_Archive::parseFileFilter($post['filter-files']) : '';
            ...
            $mtemplate->notes = str_replace($unwanted_chars, '', esc_html($post['package-notes']));
